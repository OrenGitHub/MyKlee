    ...
    bool &hasSolution)
{
  ...
  if (success) {
    logBuffer << queryCommentSign
              << "[OISH] Solvable: " << (hasSolution ? "true" : "false") << "\n";
    if (hasSolution) {
      ...
      for (std::vector<const Array *>::const_iterator i = objects.begin(),
        ...
        logBuffer << queryCommentSign << "[OISH] = " << OrenIshShalom << " " << array->name << " = [";
        
        for (int d=0;d<values[0].size() - 4;d++)
        {
		    if ((values[0].size() > 4)           && 
		    	(((char) values[0][d+0]) == '1') && 
		    	(((char) values[0][d+1]) == '2') && 
		    ...
		    {
		    	OrenIshShalom = OrenIshShalom;
		    	goto GET_OUT_OF_THIS_LOOP;
		    }
		}

GET_OUT_OF_THIS_LOOP:
