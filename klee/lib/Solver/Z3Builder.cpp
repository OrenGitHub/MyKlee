...
void custom_z3_error_handler(Z3_context ctx, Z3_error_code ec) {
  ::Z3_string errorMsg =
      ...
      // Z3 4.4.1
      Z3_get_error_msg(ctx, ec);
      // OISH COMMENTED THIS OUT: Z3_get_error_msg(ec);
#endif
...
Z3ASTHandle Z3Builder::constructActual(ref<Expr> e, int *width_out) {
  ...
  case Expr::Constant:
  {
    ...
    ConstantExpr *CE = cast<ConstantExpr>(e);
 	llvm::errs() << "**************************************************\n";
  	llvm::errs() << "* Expr::Constant(" << CE->getZExtValue(32) << ") *\n";
  	llvm::errs() << "**************************************************\n";
  ...
  case Expr::Add:
  {
    ...
    /*********************************************/
    /* OISH making sure the strins API works !!! */
    /*********************************************/
    /*********************************************/
    Z3_mk_seq_length(ctx,NULL);/*     strlen     */
    /*********************************************/
    Z3_mk_str_to_int(ctx,NULL);/*      atoi      */
    /*********************************************/
    Z3_mk_int_to_str(ctx,NULL);/*      itoa      */
    /*********************************************/
    Z3_mk_seq_at(ctx,NULL,NULL);/*     CharAt    */
    /*********************************************/
    /************************************************/
    Z3_mk_seq_extract(ctx,NULL,NULL,NULL);/* Substr */
    /************************************************/
    /***************************************************/
    Z3_mk_seq_index(ctx,NULL,NULL,NULL); /* firstIdxOf */
    /***************************************************/
    /*********************************************/
    /* OISH making sure the strins API works !!! */
    /*********************************************/
  ...
  {  
    ...
    if (*width_out == 1)
    {
      if (ConstantExpr *CE = dyn_cast<ConstantExpr>(ee->left))
      {
        ...
        Z3ASTHandle result2 = notExpr(right);
        return result2;
      }
      else
      {
        Z3ASTHandle result3 = iffExpr(left, right);
        return result3;
      }
    }
    else
    {
      *width_out = 1;
      Z3ASTHandle result4 = eqExpr(left, right);
      return result4;
    }
  }
